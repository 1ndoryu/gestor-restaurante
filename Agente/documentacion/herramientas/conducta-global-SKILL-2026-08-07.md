...
> **Copia documental de la skill instalada en `C:/Users/Owner/.agents/skills/conducta-global/SKILL.md`.**
> La fuente canónica es el archivo instalado del perfil del usuario; esta copia se mantiene para
> versionar el contenido en el repo. No editar esta copia como si fuera la fuente.
...
### 6.1. Limitación conocida: `str_replace` no edita archivos ignorados por Git

**Hecho confirmado (2026-08-07):** las herramientas de edición/búsqueda del agente pueden excluir archivos
ignorados por Git. En concreto, sobre un archivo ignorado (`.gitignore`) que existe físicamente:

- `str_replace` falla con «The file does not exist» aunque el archivo exista;
- `read_files` y las búsquedas no devuelven su contenido (BLOCKED / sin resultados);
- `write_file` sí puede crearlo o sobrescribirlo completo.

**Consecuencia de no respetar esta limitación:** el agente recurre a `write_file` para «tocar una línea» y
reescribe el archivo entero, arriesgando borrar contenido no incluido o romper formato. Eso complica el
diagnóstico, ensucia el diff y puede dañar config, fixtures o tooling.

**Regla operativa obligatoria:**

1. Si el archivo a editar está ignorado por Git (comprobar con `git check-ignore -v <ruta>`), no usar
   `write_file` completo para un cambio parcial.
2. Editar de forma quirúrgica vía terminal: reemplazo exacto con verificación previa de que el patrón
   aparece exactamente una vez (assert), o regeneración del contenido solo si el cambio es estructural y
   queda verificado con diff.
3. Verificar con `git diff` (si procede) o lectura directa que solo cambió lo previsto.
4. Si la edición parcial no es posible de forma segura, detenerse y documentar el bloqueo; no inventar
   alternativas que reescriban archivos a ciegas.
5. Esta limitación es una razón más para que la configuración/tooling necesarios vivan dentro del worktree
   de la tarea y no en archivos locales ignorados del checkout.

**No se debe:** quitar entradas de `.gitignore` para «desbloquear» herramientas, desactivar globalmente
protecciones, ni reescribir archivos completos como práctica habitual.
...
un repositorio concreto. La instalación vigente declara `version: 1.1.0` y `updated: 2026-08-07` en su
...
v1.1.0 (2026-08-07): se añadió la sección 6.1 sobre la limitación de `str_replace`/búsquedas con archivos
ignorados por Git y la regla de edición quirúrgica vía terminal, tras confirmarla en vivo al corregir el
quality gate de un consumidor.
