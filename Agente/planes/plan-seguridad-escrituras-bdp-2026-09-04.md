...
| H-S5-01 | Q2.1–Q2.13 | A6/A12 | Todo `Api` (incl. 4xx: payload inválido/conflicto) se clasificaba como **transitorio** → reintento automático + auditoría `ambiguo`. Un 422 es un rechazo **definitivo**: reintentarlo en bucle es inútil y enmascara un dato local roto. **CORREGIDO (2026-09-05):** nuevo estado `rechazado` (migración `20260905100000`, CHECK ampliado) — 4xx → `rechazado` sin consumir reintentos, flush automático no la toca (solo manual, D2), auditoría `error` (definitivo, no `ambiguo`), re-edición local la refresca a `pendiente` (M19); 5xx sigue transitorio. El cuerpo del 4xx permanece redactado ([287A-4]: status + tamaño, sin fuga). | Defecto real (medio) → corregido | `bdp_push.rs`, `tests/bdp_push.rs`, `bdp_simulator_integration.rs` |
...
| S5 | Payload inválido (tipos, precios negativos, IDs inexistentes, IVA fuera de rango): error honesto, cero daño | wiremock | **PASS 2026-09-05: push 16/0** (incl. nuevo `flush_payload_invalido_marca_rechazado_sin_reintento` end-to-end: wiremock 422 → fila `rechazado` con reintentos=0, `ultimo_error` honesto con status 422 y cuerpo redactado [287A-4], auditoría `error` (NO `ambiguo`), segundo flush automático no toca la fila, una sola llamada HTTP, y re-edición local la refresca a `pendiente`) **+ simulador 33/0** (incl. nuevo `simulator_payload_invalido_422_no_crea_fantasma`: fault `http_status: 422` en alta de artículo → `Api 422` y el artículo NO existe en la exportación posterior — cero filas fantasma) **+ lib 166/0** (3 tests nuevos de clasificación: 400/409/422 → `rechazado` sin reintento; 503 sigue transitorio). **Defecto real corregido: H-S5-01** (estado `rechazado`, migración `20260905100000`) |
...
- **Estado:** Fase 1 cerrada (H-W-1/H-W-3 corregidos con prueba, 169 verdes). **Fase 2: S1 PASS**
  ...
  `create_order`. **Fase 2: S5 PASS** (2026-09-05): push 16/0 con el nuevo test end-to-end
  `flush_payload_invalido_marca_rechazado_sin_reintento` (422 → `rechazado` sin reintentos,
  auditoría `error`, flush automático no toca la fila, una sola llamada HTTP, re-edición
  refresca a `pendiente`) + simulador 33/0 (`simulator_payload_invalido_422_no_crea_fantasma`:
  fault 422 → `Api 422` y cero filas fantasma en el simulador) + lib 166/0 (3 tests de
  clasificación). **Defecto real corregido: H-S5-01** — estado `rechazado` para 4xx
  definitivos (migración `20260905100000`; 5xx sigue transitorio; cuerpo redactado [287A-4]).
