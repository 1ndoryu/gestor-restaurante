...
- **CORRECCIÓN DE HALLAZGOS + F2 / Ronda 2 EJECUTADAS (2026-09-03):** los 4 hallazgos de la tabla §10b
  ...
  fail-closed compartido `src/handlers/bdp_guard.rs` en 9 handlers directos-BDP + botón Sync BDP de
  Plano de Sala oculto; rechazo en ~10 ms sin red), H-P1-03 (misma clase en `bdp_backup.rs`: 3
  handlers gateados + UI de PanelBdpBackup deshabilitada en standalone; ruta local intacta),
  H-P1-04 (SQL obsoleto de snapshot local de ventas alineado al esquema real con `total` derivado;
  snapshot verificado por API y UI). Ronda 2 (cruce código→checklist por dominio) cerrada: todos los
  handlers/services BDP inventariados — `ventas.rs` gateado por M12/armado, `bdp_catalogo.rs`
  local-only, colas/auditoría sin red — sin funcionalidad implementada sin caso; tipo de defecto de
