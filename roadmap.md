...
| 1e  | **Deploy a producción + correcciones de intuitividad** (dudas de Guillermo: demo, snapshots, botones pago/factura, conciliar, cancelar, mapeo, importar/enviar) — plan activo: `Agente/planes/plan-deploy-produccion-intuitividad-2026-08-07.md`. Corregir en local, validar, deploy vía coolify-manager-rs (`deploy --name glory-rest --update`) y verificar en producción. | Ninguno (la suscripción WebLink la activa Guillermo en paralelo) | ~2-4h |
| 1f  | **048A-22 — Reproducibilidad de Sentinel y del gate coordinado**: alinear gitlink/manifiesto/lock, corregir contratos de plataforma y manifiesto de etapas, y documentar la simplificación posterior. Plan: `Agente/planes/plan-correccion-auditoria-sentinel-2026-08-08.md`. | Gate base conserva deuda ajena al repin; debe quedar clasificada, no ocultada | En curso |
