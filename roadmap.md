...
| 1e  | **Deploy a producción + correcciones de intuitividad** (dudas de Guillermo: demo, snapshots, botones pago/factura, conciliar, cancelar, mapeo, importar/enviar) — plan activo: `Agente/planes/plan-deploy-produccion-intuitividad-2026-08-08.md` (incluye borrador de respuesta al cliente). Corregir en local, validar, deploy vía coolify-manager-rs (`deploy --name glory-rest --update`) y verificar en producción. | Ninguno (la suscripción WebLink la activa el dueño/cliente en paralelo) | ~2-4h deploy + ~12h UI en tarea aparte |
