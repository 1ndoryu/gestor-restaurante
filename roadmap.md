...
| 169A-1 | **Auditar ciclo de compras locales en `bdp_audit_log`** — hallazgo P9.1 (2026-09-16): crear/pasar-a-borrador/conciliar albarán (serie L) y crear gasto por conciliación NO escriben auditoría (`bdp_purchase_note.rs` y `gasto.rs` sin INSERT en `bdp_audit_log`; audit con filtro purchase/albarán/compra/reconcile = 0 de 41). Rompe la regla "cero escrituras reales sin auditoría" en local. Alcance: INSERT `albaran_local_crear/draft/reconcile` + `gasto_local_crear` con `origen_operacion='local'` + tests + Historial los muestra. | Ninguno (local) | Pendiente |
