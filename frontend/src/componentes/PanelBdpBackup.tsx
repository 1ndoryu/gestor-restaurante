...
import {
  useBdpSnapshots,
  useBdpAudit,
  useCreateSnapshotCompleto,
  useCreateSnapshotParcial,
  useCreateSnapshotGlory,
  useDeleteSnapshot,
  useRestoreSnapshot,
  useSetSyncMode,
  type BdpSnapshot,
...
function resultadoBadge(resultado: string) {
  if (resultado === 'ok') return <Badge variant="default" className="bg-green-600">OK</Badge>;
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Textarea
          placeholder="Ej: antes de migración de artículos..."
          value={notas}
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          onClick={() => crearCompleto.mutate(notas || undefined, {
            onSuccess: () => { toast.success('Snapshot completo creado'); setNotas(''); },
      ...
      <div className="border rounded-md p-3 space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {SNAPSHOT_TIPOS_BDP.map((t) => (
            <Badge
              key={t}
              variant={tiposSeleccionados.includes(t) ? 'default' : 'outline'}
              className="cursor-pointer select-none"
              onClick={() => toggleTipo(tiposSeleccionados, setTiposSeleccionados, t)}
            >
              {t}
            </Badge>
          ))}
        ...
        <Button
          ...
          onClick={() => crearParcial.mutate(
            { tipos: tiposSeleccionados, notas: notas || undefined },
            {
              onSuccess: () => { toast.success('Snapshot parcial creado'); setNotas(''); setTiposSeleccionados([]); },
              onError: (e: unknown) => toast.error('Error', { description: String(e) }),
            }
          )}
        >
      ...
      <div className="border rounded-md p-3 space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {SNAPSHOT_TIPOS_GLORY.map((t) => (
            <Badge
              key={t}
              variant={tiposGlory.includes(t) ? 'default' : 'outline'}
              className="cursor-pointer select-none"
              onClick={() => toggleTipo(tiposGlory, setTiposGlory, t)}
            >
              {t}
            </Badge>
          ))}
        ...
        <Button
          ...
          onClick={() => crearGlory.mutate(
            { tipos: tiposGlory, notas: notas || undefined },
            {
              onSuccess: () => { toast.success('Snapshot Glory creado'); setNotas(''); setTiposGlory([]); },
              onError: (e: unknown) => toast.error('Error', { description: String(e) }),
            }
          )}
        >
  ...
  if (snapshots.length === 0) {
    return (
      <p className="text-sm text-muted-foreground py-4 text-center">
        No hay snapshots todavía. Crea uno para empezar.
      </p>
    );
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Snapshots, restauración y auditoría de la sincronización con BDP.
            </CardDescription>
          </div>
