 ...
 * [237A-3] Añadido props soloArticulos / soloMapeosTecnicos para renderizar
...
interface ConfigBdpMapeosProps {
  ...
  /** Si true, solo renderiza la tabla de artículos (catálogo). */
  soloArticulos?: boolean;
  /** Si true, solo renderiza los mapeos técnicos (tender, canales, etc.) sin catálogo ni polling. */
  soloMapeosTecnicos?: boolean;
}

function ConfigBdpMapeos({ config, cambiarCampo, soloArticulos, soloMapeosTecnicos }: ConfigBdpMapeosProps) {
  ...
  /* [237A-3] Modo soloArticulos: solo tabla de catálogo + sync */
  if (soloArticulos) {
    return (
  ...
  /* [237A-3] Modo soloMapeosTecnicos: tender, canales, defaults, exigir cliente */
  if (soloMapeosTecnicos) {
    return (
      <div className="flex flex-col gap-4">
        {mapeoError && (
  ...
  /* Modo completo legacy (sin flags) — renderiza todo */
  return (
    <div className="flex flex-col gap-4">
      {mapeoError && (
        <p className="text-xs text-destructive">{mapeoError}</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="flex items-center justify-between gap-4 rounded-md border p-3">
          <div>
            <Label htmlFor="bdp-auto-sync-customers">Exigir cliente BDP confirmado</Label>
            <p className="text-xs text-muted-foreground">Si está activo, una venta con cliente sin código BDP se bloquea.</p>
          </div>
          <Switch
            id="bdp-auto-sync-customers"
            checked={config.bdp_auto_sync_customers}
